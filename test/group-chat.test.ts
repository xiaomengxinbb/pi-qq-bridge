...
import { startMockQQServer, groupAtMessageEvent, c2cMessageEvent } from "./mock-qq-server.ts";
...
test("网关：GROUP_AT_MESSAGE_CREATE → QQInboundMessage(type=group, groupOpenId)", async () => {
	...
	try {
		...
		await gateway.start();
		mock.sendEvent(...Object.entries(groupAtMessageEvent({ id: "g1", group_openid: "group_openid_x", content: "@机器人 看下这个" })).map(([, v]) => v) as [string, unknown]);
...
test("router：allowGroups 授权的群消息 → 群会话 → 群回复路径", async () => {
	...
	try {
		...
		await gateway.start();
		mock.sendEvent(...Object.entries(groupAtMessageEvent({ id: "g_reply" })).map(([, v]) => v) as [string, unknown]);
		...
		assert.deepEqual(registry.created, ["group:group_openid_1"], "群会话作用域 = group_openid");
		await gateway.stop();
	} finally {
		await mock.close();
	}
...
test("router：未授权群 → 拒绝回复（不产生访问申请）", async () => {
	...
	try {
		...
		assert.equal(runs, 0);
		assert.equal(mock.messages.length, 1);
		assert.match(String(mock.messages[0]?.body.content ?? ""), /没有权限/, "群聊不产生申请码，直接拒绝");
		await gateway.stop();
	} finally {
		await mock.close();
	}
...
test("router：私聊与群聊会话隔离（不同作用域）", async () => {
	...
	try {
		...
		gateway.onInbound((msg) => router.handleInbound(msg));
		await gateway.start();
		mock.sendEvent(...Object.entries(groupAtMessageEvent({ id: "g1" })).map(([, v]) => v) as [string, unknown]);
		await new Promise((r) => setTimeout(r, 200));
		mock.sendEvent(
			...Object.entries(c2cMessageEvent({ id: "c1", author: { user_openid: "user_allowed" } })).map(([, v]) => v) as [string, unknown],
		);
		await new Promise((r) => setTimeout(r, 200));
		assert.deepEqual(registry.created.sort(), ["group:group_openid_1", "private:user_allowed"].sort());
		await gateway.stop();
	} finally {
		await mock.close();
	}
...
test("群聊命令：allowInGroups=false 时管理员也无法在群内执行 mutation", async () => {
	...
	try {
		...
		await gateway.start();
		// 群内管理员发 /new（mutation）
		mock.sendEvent(...Object.entries(groupAtMessageEvent({ id: "g_cmd", content: "/new" })).map(([, v]) => v) as [string, unknown]);
		await new Promise((r) => setTimeout(r, 300));
		assert.equal(mock.messages.length, 1);
		assert.match(String(mock.messages[0]?.body.content ?? ""), /群聊管理命令默认关闭/);
		await gateway.stop();
	} finally {
		await mock.close();
	}
