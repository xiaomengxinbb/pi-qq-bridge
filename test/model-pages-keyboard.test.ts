...
import { buildModelPage, normalizeModelPageSize, MAX_MODEL_PAGE_SIZE } from "../src/model-pages.ts";
...
test("model-pages：每页 6 个（键盘 5 行 - 2 保留行 × 2 列）", () => {
	...
	assert.deepEqual(page.keyboardRows[4], [{ label: "返回帮助", command: "/help" }]);
...
test("model-pages：末页与翻页边界", () => {
	...
	assert.equal(page1.keyboardRows.length, 3, "单页时无翻页行：2 行模型 + 1 行帮助");
});
...
test("qq-keyboard：两列按钮 + permission.type=2 + unsupport_tips", () => {
	...
	const kb = buildCommandKeyboard(m, [
		[{ label: "状态", command: "/status" }, { label: "新会话", command: "/new" }],
	]);
	assert.ok(kb);
	...
	assert.deepEqual(button.action.permission, { type: 2 }, "不能使用 openid 作为 specify_user_ids");
...
test("qq-keyboard：行/列/标签长度裁剪 + 空输入返回 undefined", () => {
	...
	// 6 行 → 只取 5 行
	const kb = buildCommandKeyboard(m, Array.from({ length: 6 }, (_, i) => [{ label: `B${i}`, command: `/x${i}` }]));
	assert.equal(kb?.content.rows.length, 5);
	// 标签超长裁剪
	const long = buildCommandKeyboard(m, [[{ label: "超长标签".repeat(10), command: "/status" }]]);
	assert.equal(long?.content.rows[0]?.buttons[0]?.render_data.label.length, 20);
	// 空行
	assert.equal(buildCommandKeyboard(m, []), undefined);
	assert.equal(buildCommandKeyboard({ ...m, userOpenId: "" }, [[{ label: "x", command: "/x" }]]), undefined);
});
