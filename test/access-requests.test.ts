...
import { QQAccessRequestStore, normalizeAccessRole } from "../src/access-requests.ts";
import { msg } from "./helpers.ts";

const T0 = 1_000_000;

test("admit：私聊产生 6 位 code；群聊/冷却/满容量被压制", () => {
	const store = new QQAccessRequestStore({ ttlMs: 600_000, maxPending: 2, denyCooldownMs: 3_600_000 });
	...
	// 群聊不产生申请
	assert.equal(store.admit(msg({ id: "m2", type: "group", groupOpenId: "g1", userOpenId: "user_a" }), T0).suppressed, true);
	// 拒绝冷却
	...
	assert.equal(store.admit(msg({ id: "m4", userOpenId: "user_b" }), T0 + 100).suppressed, true, "冷却期内压制");
	assert.equal(store.admit(msg({ id: "m5", userOpenId: "user_b" }), T0 + 3_600_001).suppressed, false, "冷却过期后可再申请");
});
...
test("redact：申请不保存正文与附件", () => {
	...
	const admission = store.admit(
		msg({
			...
			userOpenId: "user_a",
			text: "给我执行 rm -rf /",
			attachments: [{ url: "https://evil.example/x.png", filename: "x.png", size: 1, contentType: "image/png" }],
		}),
		T0,
	);
...
test("TTL 过期自动 purge；maxPending 满则压制", () => {
	...
	// 容量满
	assert.equal(store.admit(msg({ id: "m3", userOpenId: "user_c" }), T0).suppressed, true);
	// 过期后容量释放
	assert.equal(store.admit(msg({ id: "m4", userOpenId: "user_c" }), T0 + 600_001).suppressed, false);
	assert.equal(store.list(T0 + 600_001).length, 1, "过期申请被 purge，仅剩新申请");
});
...
test("approve/deny：移除申请；deny 设置冷却", () => {
	...
	assert.equal(store.approve(code, T0 + 200), undefined, "已消费的 code 不可重复批准");
	// deny 路径
	const admission2 = store.admit(msg({ id: "m2", userOpenId: "user_b" }), T0);
	store.deny(admission2.request!.code, T0);
	assert.equal(store.admit(msg({ id: "m3", userOpenId: "user_b" }), T0 + 500).suppressed, true);
});
