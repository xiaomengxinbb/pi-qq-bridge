...
const target = { type: "private" as const, userOpenId: "openid_1", msgId: "m1" };
...
test("uploadMediaChunked：3MiB 文件走 prepare→3 块 PUT→finish", async () => {
	...
	try {
		...
		const result = await api.uploadMediaChunked(target, 4, "big.bin", fileSize, (_offset, length) => {
		...
		assert.equal(mock.partPuts.reduce((a, b) => a + b, 0), fileSize, "字节总数一致");
	} finally {
		await mock.close();
	}
});
...
test("uploadMediaChunked：文件大于 maxParts × blockSize 时截断到 maxParts", async () => {
	...
	try {
		...
		const result = await api.uploadMediaChunked(target, 4, "huge.bin", fileSize, (_o, l) => Promise.resolve(new Uint8Array(l)), {
			maxParts: 3,
		});
...
test("uploadMediaChunked：prepare 失败（端点 404）→ QQApiError", async () => {
	...
	try {
		...
		await assert.rejects(
			() =>
				api.uploadMediaChunked(target, 4, "x.bin", 1024, () => Promise.resolve(new Uint8Array(1024)), {
