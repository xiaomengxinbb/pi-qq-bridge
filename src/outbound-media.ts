...
export class QQOutboundDeliveryContext {
	...
	async sendLocalFile(inputPath: string, requestedKind: QQOutboundKind = "auto"): Promise<QQOutboundDeliveryRecord> {
		...
		try {
			...
			const maxBytes = mediaKind === "image" ? this.options.config.outboundMedia.maxImageBytes : this.options.config.outboundMedia.maxFileBytes;
			if (bytes > maxBytes) throw new QQOutboundMediaError("file_too_large", `文件超过大小限制`);
			...
			if (err instanceof QQOutboundMediaError) {
				if (!this.recordsValue.some((record) => record.filename === filename && record.errorCode === err.code)) {
					this.recordsValue.push(this.failureRecord(filename, mediaKind, bytes, err.code, err.message));
				}
				throw err;
			}
...
function safeFilename(path: string): string {
	return (
		basename(path.replaceAll("\\", "/")).replace(/[\u0000-\u001f\u007f]/g, "").slice(0, 120) || "file"
	);
}
