...
export function classifyAttachment(attachment: { filename: string; contentType: string }): QQAttachmentKind {
	const name = safeOriginalFilename(attachment.filename).toLowerCase();
	const contentType = (attachment.contentType ?? "").toLowerCase();
	if (contentType.startsWith("image/") || /\.(png|jpe?g|gif|webp|bmp)$/.test(name)) return "image";
	if (contentType.startsWith("audio/") || contentType === "voice" || /\.(silk|amr|wav|mp3|ogg|m4a|flac)$/.test(name)) return "audio";
...
function isRetryable(err: unknown): boolean {
	if (err instanceof AttachmentDownloadError) {
		return err.code.startsWith("retryable_http_") || err.code === "network_error" || err.code === "dns_failed";
	}
	return false;
}
...
function looksTextual(buffer: Buffer): boolean {
	...
	for (const byte of sample) {
		if (byte === 0 || (byte < 0x09) || (byte > 0x0d && byte < 0x20)) controls += 1;
	}
